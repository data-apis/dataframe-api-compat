...
def test_column_divmod(library: str) -> None:
    ...
    compare_column_with_reference(result.col("result"), [0, 0, 0], dtype=pdx.Int64)
    # remainder
...
def test_expression_divmod_with_scalar(library: str) -> None:
    ...
    # quotient
    result = df.assign(result_quotient.rename("result"))
    compare_column_with_reference(result.col("result"), [0, 1, 1], dtype=pdx.Int64)
    # remainder
    result = df.assign(result_remainder.rename("result"))
    compare_column_with_reference(result.col("result"), [1, 0, 1], dtype=pdx.Int64)
