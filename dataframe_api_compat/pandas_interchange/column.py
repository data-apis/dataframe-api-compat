...
class PandasColumn(Column):
    ...
    def __init__(self, column: pd.Series, allow_copy: bool = True) -> None:
        ...
        if not isinstance(column, pd.Series):
            msg = f"Columns of type {type(column)} not handled yet"
    ...
    def dtype(self) -> tuple[DtypeKind, int, str, str]:
        ...
        if isinstance(dtype, pd.CategoricalDtype):
            ...
            msg = "Non-string object dtypes are not supported yet"
            raise NotImplementedError(msg)
        else:
    ...
    def _dtype_from_pandasdtype(self, dtype) -> tuple[DtypeKind, int, str, str]:
        ...
        if kind is None:
            ...
            msg = f"Data type {dtype} not supported by interchange protocol"
    ...
    def describe_categorical(self):
        ...
        if self.dtype[0] != DtypeKind.CATEGORICAL:
            msg = "describe_categorical only works on a column with categorical dtype!"
            raise TypeError(
                msg,
            )
    ...
    def describe_null(self):
        ...
        try:
            ...
            msg = f"Data type {kind} not yet supported"
            raise NotImplementedError(msg)
    ...
    def get_chunks(self, n_chunks: int | None = None):
        ...
        if n_chunks and n_chunks > 1:
            ...
            for start in range(0, step * n_chunks, step):
                yield PandasColumn(
                    self._col.iloc[start : start + step],
                    self._allow_copy,
                )
        else:
            yield self
    ...
    def get_buffers(self) -> ColumnBuffers:
        ...
        with contextlib.suppress(NoBufferPresent):
    ...
    def _get_data_buffer(
        ...
        if self.dtype[0] in (
            ...
            msg = f"Data type {self._col.dtype} not handled yet"
            raise NotImplementedError(msg)
    ...
    def _get_offsets_buffer(self) -> tuple[PandasBuffer, Any]:
        ...
        if self.dtype[0] == DtypeKind.STRING:
            ...
            msg = "This column has a fixed-length dtype so it does not have an offsets buffer"
