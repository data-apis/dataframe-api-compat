...
def from_dataframe(df, allow_copy: bool = True) -> pd.DataFrame:
    ...
    if not hasattr(df, "__dataframe__"):
        msg = "`df` does not support __dataframe__"
...
def _from_dataframe(df: DataFrameXchg, allow_copy: bool = True):
    ...
    if not allow_copy and len(pandas_dfs) > 1:
        msg = "To join chunks a copy is required which is forbidden by allow_copy=False"
        raise RuntimeError(
            msg,
        )
    if not pandas_dfs:
...
def protocol_df_chunk_to_pandas(df: DataFrameXchg) -> pd.DataFrame:
    ...
    for name in df.column_names():
        if not isinstance(name, str):
            msg = f"Column {name} is not a string"
            raise ValueError(msg)
        if name in columns:
            msg = f"Column {name} is not unique"
            raise ValueError(msg)
        ...
        if dtype in (
            ...
            msg = f"Data type {dtype} not handled yet"
...
def categorical_column_to_series(col: Column) -> tuple[pd.Series, Any]:
    ...
    if not categorical["is_dictionary"]:
        msg = "Non-dictionary categoricals not supported yet"
        raise NotImplementedError(msg)
    ...
    if hasattr(cat_column, "_col"):
        ...
        msg = "Interchanging categorical columns isn't supported yet, and our fallback of using the `col._col` attribute (a ndarray) failed."
    ...
    values = categories[codes % len(categories)] if len(categories) > 0 else codes
...
def string_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    ...
    if null_kind not in (
        ...
        msg = f"{null_kind} null kind is not yet supported for string columns."
        raise NotImplementedError(
            msg,
        )
...
def parse_datetime_format_str(format_str, data) -> pd.Series | np.ndarray:
    ...
    if date_meta:
        ...
        if unit == "D":
            ...
            msg = f"Date unit is not supported: {unit}"
            raise NotImplementedError(msg)
        return data

    msg = f"DateTime kind is not supported: {format_str}"
    raise NotImplementedError(msg)
...
def buffer_to_ndarray(
    ...
    *,
    ...
    if column_dtype is None:
        msg = f"Conversion for {dtype} is not yet supported."
        raise NotImplementedError(msg)
...
def set_nulls(
    ...
    if null_kind == ColumnNullType.USE_SENTINEL:
        ...
        msg = f"Null kind {null_kind} is not yet supported."
        raise NotImplementedError(msg)
